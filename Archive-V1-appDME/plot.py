...
def initialize_system(timestamp: str) -> Tuple[torch.device, Path, Path, torch.Tensor, Dict, torch.Tensor]:
    ...
    rho = torch.load(data_dir/'rho_n.pt', weights_only=False)
    Mesh_data = torch.load(data_dir/'Mesh.pt', weights_only=False)
    P = torch.load(data_dir/'P.pt', weights_only=False)
...
def main():
    ...
    device, data_dir, fig_dir, rho, Mesh_data, P = initialize_system(timestamp)  
